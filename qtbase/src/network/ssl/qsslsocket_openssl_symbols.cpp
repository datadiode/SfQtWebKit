...
static QPair<QSystemLibrary*, QSystemLibrary*> loadOpenSslWin32()
{
    ...
    tryToLoadOpenSslWin32Library(QLatin1String("libssl-1_1"), QLatin1String("libcrypto-1_1"), pair);
...
QDateTime q_getTimeFromASN1(const ASN1_TIME *aTime)
{
    ...
    char *const pStringBad = pString + lTimeLength;
    ...
    if (aTime->type == V_ASN1_UTCTIME) {
        ...
        if ((*pString == 'Z') || (*pString == '-') || (*pString == '+')) {
            ...
            *pBuffer++ = *pString++;
            ...
            if (*pString == '.') {
                pString++;
                if (pString >= pStringBad) // Oh no, cannot dereference (see below).
                    return QDateTime();
                while ((*pString >= '0') && (*pString <= '9')) {
                    pString++;
                    if (pString >= pStringBad) // No and no.
                        return QDateTime();
                }
            }
        }
