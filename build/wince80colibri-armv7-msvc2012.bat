...
SET WinTee="WinTee.exe" -file "%QT_BUILD%\logfile.log" -stopwatch
...
SETLOCAL
	...
	%WinTee% -name "configure" ^
	CALL %QT_SOURCE%\configure.bat -opensource -confirm-license -rtti -no-harfbuzz -ssl -openssl -icu -opengl es2 -nomake tests -nomake examples -nomake tools -skip webengine -skip translations -platform win32-msvc2012 -xplatform %QMAKESPEC% -release
...
if errorlevel 0 %WinTee% -append -name "build" nmake.exe
