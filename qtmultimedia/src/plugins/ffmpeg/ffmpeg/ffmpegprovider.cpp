...
void FFmpegProvider::setState(FFmpegProvider::State s)
{
    if (_play_state != s) {
        _play_state = s;
        if (_decoder)
            _decoder->requestPlayState(DecoderThread::toDecoderState(s));
        _control->onStateChange(s);
    }
}
...
void sdl_audio_callback(void *user_data, uint8_t *stream, int len)
{
    ...
    if (buf) {
        ...
        double pow2 = log(SDL_MIX_MAXVOLUME);
        double div = 100 / pow2;
        double exp_vol = pow(2, vol_p / div);   // min = 1, max = 128
        int v = static_cast<int>(floor(exp_vol));
        if (exp_vol < 1.01) { v = 0; }
...
{
    if (_decoder)
        _decoder->waitForState(DecoderThread::toDecoderState(s));
}
